<?php
	...
	class Event_Invitation extends Noti_Sendable{
		...
		public function sendInvitation($user_id, $user_id_get_hash, $event_id){
			...
			$event = new Event();
			if($user_id_get !== false && $user_id != $user_id_get &&  $event->isEventEditableByUser($event_id, $user_id)){
		...
		public function isUserInvitedInEvent($user_id_get, $event_id){
			$stmt = $this->connection->prepare("SELECT `id` FROM `$this->table_name` WHERE `user_id_get`=? AND (`process`='n' || `process` = 'i')AND `event_id`=? LIMIT 1");
			if($stmt){
